...
const validateReferentialIntegrity = (
  ...
  const subCategoryIdsByCategory = new Map<string, Set<string>>(
    categories.map((cat) => [
      cat.id,
      new Set((cat.subCategories ?? []).map((sub) => sub.id)),
    ]),
  )
...
const doLoad = async (): Promise<SiteConfig> => {
  ...
  const categories = categoriesSchema.parse(categoriesData) as Category[]
  const links = linksSchema.parse(linksData) as Link[]
  const searchConfig = searchConfigSchema.parse(searchData) as SearchConfig
  const settings = siteSettingsSchema.parse(settingsData) as SiteSettings
